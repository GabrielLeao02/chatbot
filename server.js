...
/*
  Cada entry: {
    ...
    status: 'starting'|'qr'|'authenticated'|'ready'|'failed'|'disconnected',
 ...
 */
app.get('/instance/:companyId/:peopleId/status', (req, res) => {
 ...
 *   Se precisar só do PNG puro, você pode retornar base64 cortando o prefixo.
 */
app.get('/instance/:companyId/:peopleId/qr', (req, res) => {
  const { companyId, peopleId } = req.params;
  const key = keyFrom(companyId, peopleId);
  const entry = instances.get(key);
